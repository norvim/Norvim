...
$('walletTopupForm')?.addEventListener('submit', async e => {
  ...
  try { const r=await api('/api/account/wallet/topup',{method:'POST',body:JSON.stringify({amount, phone: $('walletTopupPhone')?.value.trim()})}); alert((r.message || 'Complete the M-PESA prompt on your phone.') + (r.paystackMode === 'test' ? '\n\nPaystack TEST mode uses the test M-PESA number +254710000000. No real M-PESA money is charged.' : '')); $('walletTopupAmount').value=''; pollWallet(r.reference); }
...
$('walletWithdrawForm')?.addEventListener('submit', async e => {
  ...
  try {
    const r = await api('/api/account/wallet/withdraw', { method:'POST', body:JSON.stringify({ amount, phone: $('walletWithdrawPhone')?.value.trim() }) });
