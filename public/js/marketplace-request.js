...
(async () => {
  try {
    ...
    if (!catsResponse.ok) throw Error(cats.message || 'Failed to load service categories.');
    $('service').innerHTML = cats.map(c => `<option value="${esc(c._id)}">${esc(c.name)}</option>`).join('');
    if (requestedWorkerId) {
      const r = await fetch('/api/marketplace/workers/' + encodeURIComponent(requestedWorkerId));
      const w = await r.json();
      if (r.ok && w.services?.length) $('service').value = w.services[0]._id;
    }
...
$('form').addEventListener('submit', async e => {
  ...
  try {
    ...
    const d = await api('/api/marketplace/requests', {
      method:'POST',
      body:JSON.stringify({ serviceCategoryId:$('service').value, workerId:requestedWorkerId || undefined, title:$('title').value, description:$('description').value, location, budget:{ type:$('budgetType').value, amount:$('amount').value ? Number($('amount').value) : undefined } })
    });
