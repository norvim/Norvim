(() => {
  ...
  const details = {
    ...
    marketplace: { title:'Marketplace Premium', kicker:'NORVIM PLUS · MARKETPLACE', tagline:'Stand out. Win more work.', benefits:['Premium worker profile presentation','Premium badge','Priority presentation where applicable','🔔 Choose your marketplace opportunity ringtone'] }
  };
  function openModal(product){
    ...
    if(!modal){
      ...
      modal.innerHTML='<div class="premium-modal-backdrop" data-premium-close></div><div class="premium-modal-card" role="dialog" aria-modal="true" aria-labelledby="premiumModalTitle"><button class="premium-modal-close" type="button" data-premium-close aria-label="Close">×</button><div class="premium-modal-icon">♛</div><span class="premium-home-kicker" id="premiumModalKicker"></span><h2 id="premiumModalTitle"></h2><p id="premiumModalTagline"></p><ul id="premiumModalBenefits"></ul><div class="premium-ringtone-choice" id="premiumRingtoneWrap"><label for="premiumRingtone">Marketplace opportunity ringtone</label><select id="premiumRingtone"><option value="bell">Norvim Bell</option><option value="chime">Chime</option><option value="soft">Soft Alert</option><option value="alert">Alert</option><option value="none">No sound</option></select></div><div class="premium-modal-price"><strong>KSh 100</strong><span>/ 30 days</span></div><button class="premium-home-btn premium-modal-pay" id="premiumModalPay" type="button">Upgrade to Premium →</button><p class="premium-modal-message" id="premiumModalMessage" aria-live="polite"></p></div>';
    ...
    const ringtone=document.getElementById('premiumRingtone'); const wrap=document.getElementById('premiumRingtoneWrap'); if(wrap) wrap.style.display=product==='marketplace'?'block':'none'; if(ringtone) ringtone.value=localStorage.getItem('norvimPremiumRingtone')||'bell';
  ...
  async function start(product,btn){
    ...
    try{
      const r=await fetch('/api/premium/request',{method:'POST',headers:{'Content-Type':'application/json',Authorization:'Bearer '+t},body:JSON.stringify({product, notificationSound: document.getElementById('premiumRingtone')?.value || 'bell'})});
  ...
  document.addEventListener('change',e=>{if(e.target.id==='premiumRingtone') localStorage.setItem('norvimPremiumRingtone',e.target.value);});
